...
harmonic_df = None
try:
    harmonic_df = pd.read_sql("select * from harmonic_pattern_h1", engine)
except:
    pass

df = pd.read_sql("select id, symbol, open, high, low, close, date_h1 from stock_price_h1 where open_time >= 1684108800000 order by symbol asc, open_time asc", engine)
...
for symbol_name, symbol in df.groupby("symbol"):
    symbol = symbol.sort_values("date_h1", ascending=True)[["id", "symbol", "open", "high", "low", "close", "date_h1"]].reset_index(drop=True)
    last_pat = None
    try:
        hsdf = pd.read_sql(f"select max(id_X) as maxa from harmonic_pattern_h1 where symbol = '{symbol_name}'", engine)
...
out_df.to_sql("harmonic_pattern_h1", index=False, con=engine, if_exists="append") 
