...
class Sendh4psar_IndexController extends Zend_Controller_Action {
            ...
            $dataPoints = $this
                ...
                ->queryAll("SELECT `a`.*,`a`.`id` as `ID` FROM `stock_price_h4` as `a`
                        WHERE `date`<='$dateCur'
                         AND `date`>='$dateCheck'
                    AND `symbol`='{$symbol}'
                    ORDER BY `date_h4` ASC");

            if ($dataPoints) {

                $dataFlags = [];
                foreach ($dataPoints as $dt) {
                    $dataFlags[$dt['date_h4']] = $dt;
                }
                ...
                foreach ($dataPoints as $k => $a) {
                    $dateCurrent = $a['date_h4'];
                    $dateSub = date("Y-m-d H:i:s", strtotime($dateCurrent . "-4 hours"));
                    $dateSub2 = date("Y-m-d H:i:s", strtotime($dateCurrent . "-8 hours"));
...
//                        }
                        ...
                        $str[] = "UPDATE `stock_price_h4`
                                SET `psar`='{$s['psar']}',
                                    `psar_type`='{$s['psar_type']}',
                                    `psar_low`='{$s['psar_low']}',
                                    `psar_high`='{$s['psar_high']}'
                                    WHERE `ID`='{$ks}' AND `date`>='2022-08-01'";
                    }
