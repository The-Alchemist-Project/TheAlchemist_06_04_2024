...
class Knowledge_IndexController extends Zend_Controller_Action {

    var $tb_post = "tb_knowledge_post";
    var $tb_cat = "tb_knowledge_cat";
    var $tb_config = "tb_knowledge_config";
        ...
        if ($postID != 0) {
            ...
            $this->view->posts = $this
                    ...
                    ->queryAll("SELECT *,
						...
						FROM {$this->tb_post}
						WHERE status<>0  ORDER BY idx ASC, date DESC,date DESC LIMIT $limit");
            ...
            $this->view->pageBar = $this
                    ->Plugins
                    ->getPageBarDiv(($this->language == "vn" ? 'kien-thuc' : 'Knowledge') . "/?p=", $current, $total, $config['post_number'], $config['next_number'], true);
        }
    }
