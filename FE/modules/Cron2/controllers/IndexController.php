...
class Cron2_IndexController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        ...
        $whereSymbol = ["'1'='1'"];
        $whereSymbol[] = "`d1_standard`='PENDING'";
        if ($symbol) {
...
//ORDER BY `ID` LIMIT 100");
        ...
        foreach ($symboyAlls as $k => $s) {
            ...
            for ($d = strtotime($dateCheck); $d >= strtotime($dateOld); $d = strtotime("-1 day", $d)) {
                ...
                $date = date("Y-m-d", $d);
                //$date_find = strtotime ( '+1 day' , strtotime ( $date ) ) ;
                //$datePrev = date("Y-m-d", strtotime($date) . '-5 days');
                $begandate = strtotime ( '-6 day' , strtotime ( $date ) ) ;
                $postSubs = $this
                        ->Model
...
//AND `date`<= 'CURDATE()'
                ...
                if ($postSubs) {
                    ...
                    foreach ($postSubs as $r => $row) {
                        ...
                        //if ((double) $row['rsi'] > 32 && (double) $row['rsi'] < 68 ) {
                        //   continue;
                        //}
                    ...
                    foreach ($formulars as $fomular) {
                        ...
                        if ($result) {
                            ...
                            //cài đặt dữ liệu nhập vào stock_price_details
                            $dataInsert = [
                                'post_id' => $id,
                                'type_id' => $fomular['ID'],
                                ...
                                //'date_find' => $date
                                'date_find' => date("Y-m-d H:i:s" , strtotime ( '+1 day' , strtotime ( $date ) ))
                            ];
            ...
            //$this->Model->update('stock_price_details' WHERE `ID` = '$id' SET `date_find`= DATE_ADD(`date`, INTERVAL 1 DAY);)

                    //UPDATE `stock_price_details` SET `date_find`= DATE_ADD(`date`, INTERVAL 1 DAY) WHERE `id`=55
            //cập nhật ngày báo kèo
            //$this->Model->UPDATE(`stock_price_details` SET `date_find`= DATE_ADD(`date`, INTERVAL 1 DAY));
        }
