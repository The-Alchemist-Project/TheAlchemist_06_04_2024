...
//            }
            ...
            $this->view->posts = $this
                    ...
                    ->queryAll("SELECT `a`.*,
                        ...
                        IF('{$this->language}'='vn',`a`.`inventory_vn`,`a`.`inventory_en`) as `inventory`,
                        IF('{$this->language}'='vn',`a`.`address_vn`,`a`.`address_en`) as `address`,
                        IF('{$this->language}'='vn',`a`.`service_vn`,`a`.`service_en`) as `service`,
...
//                    IF('{$this->language}'='vn',`content_vn`,`content_en`) as `content`
//					FROM {$this->tb_post}
//					WHERE  date <= {$this->Plugins->time()} ORDER BY date DESC,ID");
