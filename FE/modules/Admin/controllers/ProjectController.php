...
class Admin_ProjectController extends Zend_Controller_Action {
    ...
    var $tb_post = "tb_project_post";
    var $tb_config = "tb_project_config";
    var $tb_cat = "tb_project_cat";
    var $tb_files = "tb_project_files";
    var $tb_user = "tb_user";
    var $page = 10;
    var $dirImage = "files/images/project/";
    var $dirFiles = "upload/project/";
    var $thumbsImage = "upload/project/thumbs/";
    ...
    public function editcatAction() {
        ...
        $content_vn = $this->Plugins->getEditor("content_vn", $cat['content_vn']);
        $content_en = $this->Plugins->getEditor("content_en", $cat['content_en']);
        ...
        if ( isset($_REQUEST['act_move']) ) {
            ...
            if ( !is_array($id) ) {
                ...
                foreach ( $id as $i ) {
                    ...
                    $post_files = $this->Model->get($this->tb_files, "WHERE parent_id=$i");
                    if ( $post ) {
                        unlink($this->dirImage . $post['img']);
    ...
    public function addpostAction() {
        ...
        $year = $this->Plugins->get("year", "");
        $address_vn = $this->Plugins->get("address_vn", "");
        $address_en = $this->Plugins->get("address_en", "");
        $service_vn = $this->Plugins->get("service_vn", "");
        $service_en = $this->Plugins->get("service_en", "");
        $service_info_vn = $this->Plugins->getEditor("service_info_vn", "");
        $service_info_en = $this->Plugins->getEditor("service_info_en", "");
        ...
        $this->view->service_vn = $service_vn;
        $this->view->service_en = $service_en;
        $this->view->service_info_vn = $service_info_vn;
        $this->view->service_info_en = $service_info_en;
        $this->view->year = $year;
        $this->view->date = $date;
        ...
        if ( $this->isPost ) {
            ...
            $query = $this->Model->insert($this->tb_post, array(
                ...
                service_info_en => $service_info_en,
                service_info_vn => $service_info_vn,
    ...
    public function editpostAction() {
        ...
        $year = $this->Plugins->get("year", $post['year']);
        $address_vn = $this->Plugins->get("address_vn", $post['address_vn']);
        $address_en = $this->Plugins->get("address_en", $post['address_en']);
        $service_vn = $this->Plugins->get("service_vn", $post['service_vn']);
        $service_en = $this->Plugins->get("service_en", $post['service_en']);
        $service_info_vn = $this->Plugins->getEditor("service_info_vn", $post['service_info_vn']);
        $service_info_en = $this->Plugins->getEditor("service_info_en", $post['service_info_en']);
