...
class Admin_ShareholderController extends Zend_Controller_Action {
    ...
    var $tb_user = "tb_user";
    var $page = 10;
    var $dirImage = "files/images/shareholder/";
    ...
    public function postAction() {
        ...
        if ( !isset($_REQUEST['s']) ) {
            ...
            $query_all = "SELECT post.*,
							...
							(SELECT GROUP_CONCAT(\"<a href='{$this->view->actionUrl}/?parent_id=\",cat.ID,\"'>\",cat.title COLLATE utf8_general_ci,'</a>') FROM {$this->tb_cat} as cat WHERE lang='{$_SESSION['cp_lang']}' AND post.parent_id=cat.ID LIMIT 0,1) as cat_link,( CASE WHEN sticky > 0 THEN sticky ELSE 999999 END ) as idx
							FROM {$this->tb_post} as post
						WHERE $mysql_qr lang='{$_SESSION['cp_lang']}' ORDER BY idx, date DESC LIMIT $limit";

//            $posts = $this->Model->query($query_all);
//            foreach($posts as $a){
//                $this->Model->update($this->tb_post, array(
//                    date => $this->Plugins->strtotime($a['year'].'-01-01 08:00:00')
//                    ), "ID={$a['ID']}");
//            }
    ...
    public function addpostAction() {
        ...
        $parent_id = $this->Plugins->getNum("parent_id", 0);
        $year = $this->Plugins->getNum("year", date('Y'));
        ...
        if ( $this->isPost ) {
            ...
            if(!$year){
                $this->view->message = "Năm bắt buộc phải chọn";
            }
            ...
            $file_upload = "";
            if ( $file && ( $file['tmp_name'] !== "" ) ) {
                $file_upload = $this->Plugins->upload($this->dirImage, $file);
                if ( is_int($file_upload) ) {
                    die($file_upload);
                    $this->view->message = "ERROR_UPLOAD_$file_upload";
                    return false;
            ...
            $query = $this->Model->insert($this->tb_post, array(
                ...
                title_vn => $title_vn,
                title_en => $title_en,
                //year => date('Y',$this->Plugins->strtotime($date)),
                year => $year,
                hot => $hot,
                parent_id => $parent_id,
                sticky => $sticky,
    ...
    public function editpostAction() {
        ...
        $year = $this->Plugins->getNum("year", $post['year']);
