...
class Crond1psar_IndexController extends Zend_Controller_Action {
    ...
    public function indexAction() {
        ...
        date_add($dateOld, date_interval_create_from_date_string("-25 days"));
        ...
        foreach ($symboyAlls as $k => $s) {
            $symbol = $s['symbol'];
            //echo $symbol. "<br />";
            for ($d = strtotime($dateCheck); $d > strtotime($dateOld); $d = strtotime("-1 days", $d)) {
                    ...
                    foreach ($formulars as $fomular) {
                        ...
                        if ($result) {
                            ...
                            if (preg_match('/Long - /', $type)) {
                                ...
                                WHERE `date`='{$date}' AND `symbol`='{$symbol}' AND `id`='{$id}';";
                            } else {
                                $sql .= "UPDATE `stock_price` 
                                        SET `psar_model_title`='{$type}',
