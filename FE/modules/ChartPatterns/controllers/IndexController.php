...
class ChartPatterns_IndexController extends Zend_Controller_Action {
...
//        }
    ...
    public function indexAction() {
        ...
        $table_pattern = 'harmonic_pattern_d1';
        if ($type == 'H4') {
            $table = 'stock_price_h4';
            $table_detail = 'stock_price_h4_details';
            $table_pattern = 'harmonic_pattern_h4';
            $order = 'date_h4';
            $parentTitle = 'H4';
            //cmt by tungvq 03/09/2023
            $dateSearch = '2022-03-15';
        } elseif ($type == 'H1' || $type == '30m') {
            $table = 'stock_price_h1';
            $order = 'date_h1';
            $table_pattern = 'harmonic_pattern_h1';
            $parentTitle = 'D1';
            $table_detail = 'stock_price_h1_details';
            $dateSearch = '2021-03-15';
        } else {
            $table = 'stock_price';
            $order = 'date';
            $parentTitle = 'D1';
        ...
        $posts = $this
                ...
                FROM `{$table}` as `a`
                WHERE `a`.`{$studyQuery}` > 0
                AND `a`.`{$order}`>='{$dateSearch}'
		...
		$patternsPosts = $this
                ->Model
                ->queryAll("SELECT `a`.*,`a`.`id` as `ID`
                FROM `{$table_pattern}` as `a`
            ORDER BY `date_found` DESC LIMIT 200");
		$this->view->patternsPosts = $patternsPosts;
        if ($customer) {
            $postMyLists = $this
                    ->Model
                    ->queryAll("SELECT `a`.*,`a`.`id` as `ID`
        ...
        if ($ID) {
            ...
            $patternCurrent = $this
                    ->Model
                    ->getOne($table_pattern, "WHERE  `ID`='$ID'");
        }
        
        $this->view->patternCurrent = $patternCurrent;
        ...
        $patternQueryString = "123";
        $tempPattern = $arrDatas[$postCurrent['type_id']];
        $patternQueryString = $tempPattern['title'];
        $this->view->patternQueryString = $postCurrent['model_title'] ?? $patternQueryString;
