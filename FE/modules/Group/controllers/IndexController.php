...
class Group_IndexController extends Zend_Controller_Action {
    ...
    public function itemsAction() {
        ...
        $posts = $this
                ...
                FROM `tb_social_groups` as `a`
                LEFT JOIN `customer` as `b`
                ON `a`.`user_post`=`b`.`ID`
                WHERE FIND_IN_SET('{$customer['ID']}',`a`.`member_ids`)
                    OR `user_post`='{$customer['ID']}'
                    OR `user_edit`='{$customer['ID']}'
        ...
        $members = $this
                ->Model
                ->queryAll("SELECT `a`.`ID`,`a`.`name`,`a`.`user`
                    FROM `customer` as `a`
                        WHERE `a`.`status`='1'");
    ...
    public function addmemberAction() {
        $customer = $_SESSION['cp__customer'];
        if ($this->isPost) {
            $id = $this->Plugins->getNum("group_id", "0");

            $title = $this->Plugins->get("title");
            $members = $this->Plugins->getArray("member_ids");

            $error = [];
            if (!$title) {
                $error['title'] = '+ Group name is required';
            }
            if (!$members) {
                $error['member_ids'] = '+ Member is not empty';
            }
            ...
            if ($id) {
                $group = $this->Model->getOne('tb_social_groups', "WHERE ID='$id' AND `user_post`='{$customer['ID']}'");
                if (!$group) {
                    die(json_encode([
                        'error' => false,
                        'msg' => 'Error2'
                ...
                $this->Model->update('tb_social_groups', array(
                    'title' => $title,
                    'member_ids' => $members,
                ...
                $this->Model->insert('tb_social_groups', array(
                    'title' => $title,
                    'member_ids' => $members,
            ...
            die(json_encode([
                'error' => false,
                'groupId' => $id ?? 0,
                'msg' => 'Congratulations on your created group successfully'
            ]));
        } else {
            die('Error');
        }
